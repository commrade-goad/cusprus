...
fn read_configuration() -> (Vec<String>, Vec<String>){
    ...
    let path_to_config: String = format!("{home_folder}/.config/cusprus.toml");
        if std::path::Path::new(&path_to_config).is_file() == false {
            let mut create_config = fs::File::create(&path_to_config).expect("Error : Failed to create the configuration file!");
    ...
    return (
        parse_file.pretty_name,
        parse_file.command,
            );
}

fn spawn_rofi(pretty_name: Vec<String>) -> usize {
...
fn main() {
    ...
    let value = spawn_rofi(configuration.0);
